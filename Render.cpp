...
void RenderGameplay() {
    ...
    if (paddleVisible) { deviceContext->PSSetShader(pixelShaderPaddle, nullptr, 0); deviceContext->IASetVertexBuffers(0, 1, &vertexBuffer, &stride, &offset); deviceContext->Draw(6, 0); }
    ...
    if (forceFieldActive) {
        ...
        for (int i = 0; i <= segments; i++) { float theta = (2 * 3.14159265f * i) / segments; float x = forceFieldX + cosf(theta) * forceFieldRadius; float y = forceFieldY + sinf(theta) * forceFieldRadius; circleVerts.push_back({ x, y, 0.0f }); }
        std::vector<Vertex> fanVerts; for (int i = 1; i < circleVerts.size() - 1; i++) { fanVerts.push_back(circleVerts[0]); fanVerts.push_back(circleVerts[i]); fanVerts.push_back(circleVerts[i + 1]); }

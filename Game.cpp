...
void ActivateDash() { dashActive = true; dashTimer = 15; }
void ActivateforceField() { forceFieldActive = true; forceFieldX = paddleX; forceFieldY = paddleY + paddleHeight * 0.7f; forceFieldTimer = 10; }
...
void UpdateDiffSelect() {
    ...
    if (isZPressed && !g_wasZPressed) { difficulty = selectedMenuIndex; InitGameplay(difficulty, cfgLife); currentState = GameState::STATE_GAMEPLAY; }
    if (isXPressed) { selectedMenuIndex = 0; currentState = GameState::STATE_START_MENU; }
    g_wasUpPressed = isUpPressed; g_wasDownPressed = isDownPressed; g_wasZPressed = isZPressed;
}

void UpdateIFrame() { if (iFrame) { paddleVisible = !paddleVisible; iFrameTimer -= 1; if (iFrameTimer <= 0) { iFrame = false; paddleVisible = true; } } }
...
void UpdateMenu() {
    ...
    if (isZPressed && !g_wasZPressed) {
        if (selectedMenuIndex == 0) { selectedMenuIndex = 1; currentState = GameState::STATE_DIFFICULTY_SELECT; }
        else if (selectedMenuIndex == 2) { PostQuitMessage(0); }
    }
    g_wasUpPressed = isUpPressed; g_wasDownPressed = isDownPressed; g_wasZPressed = isZPressed;
}
...
void UpdateBall() {
    ballX += ballVelX; ballY += ballVelY; ballVelY -= 0.0007f;
    if (ballX - ballSize < -0.9f) { ballX = -0.9f + ballSize; ballVelX *= -1; }
    if (ballX + ballSize > 0.9f) { ballX = 0.9f - ballSize; ballVelX *= -1; }
    if (ballY + ballSize > 1.0f) { ballY = 1.0f - ballSize; ballVelY *= -1; }
    if (ballY - ballSize < -0.72f) { ballY = -0.72f + ballSize; ballVelY *= -0.80f; combo = 0; }
    ...
    if (ballY - ballSize <= paddleY + paddleHeight && ballX >= paddleX - paddleWidth / 2 && ballX <= paddleX + paddleWidth / 2 && ballY > paddleY) {
        ...
        if (!iFrame) { iFrame = true; iFrameTimer = 60 * 5; life -= 1; }
    }
        ...
        if (hitX && hitY) {
            if (!block.iFrameBlock) {
                ...
                for (int i = 0; i < block.bulletCount; i++) {
                    if (block.bulletPattern == 0) { float spread = 1.0f; float startAngle = pAngle - (spread / 2.0f); float a = (block.bulletCount > 1) ? startAngle + (spread * i / (block.bulletCount - 1)) : pAngle; SpawnEnemyBulletAngle(block.x, block.y, a, 0.007f); }
                    else if (block.bulletPattern == 1) { float spd = 0.005f + (i * 0.002f); SpawnEnemyBulletAngle(block.x, block.y, pAngle, spd); }
                    else if (block.bulletPattern == 2) { float a = (2.0f * 3.14159265f * i) / block.bulletCount; SpawnEnemyBulletAngle(block.x, block.y, a, 0.006f); }
                    else if (block.bulletPattern == 3) { float a = i * 0.5f; float spd = 0.003f + (i * 0.0003f); SpawnEnemyBulletAngle(block.x, block.y, a, spd); }
                }
                break;
            }
        }
    ...
    for (auto& enemyBullet : enemyBullets) {
        ...
        if (hitX && hitY) { enemyBullet.active = false; break; }
    }

    float dt = 1.0f; float nearestT = 1.0f; float normalX = 0.0f, normalY = 0.0f;
    for (auto& obstacle : obstacles) {
        ...
        if (res.t < nearestT) { nearestT = res.t; normalX = res.nx; normalY = res.ny; }
        if (nearestT < 1.0f) { ballX += ballVelX * dt * nearestT; ballY += ballVelY * dt * nearestT; if (normalX != 0.0f) ballVelX = -ballVelX; if (normalY != 0.0f) ballVelY = -ballVelY; }
    }
...
void UpdateEnemyBullet() {
    for (auto& bullet : enemyBullets) {
        ...
        if (bullet.y - bullet.size < paddleY + paddleHeight && bullet.x > paddleX - paddleWidth / 2 && bullet.x < paddleX + paddleWidth / 2 && bullet.y > paddleY) {
            bullet.active = false; if (!iFrame) { iFrame = true; iFrameTimer = 60 * 5; life -= 1; combo = 0; }
        }
    }
}
...
void UpdateBlocks() {
    for (auto& b : blocks) {
        ...
        if (b.hits <= 0) { b.active = false; blocksRemaining--; }
        if (b.iFrameBlock) { b.iFrameBlockTimer -= 1; if (b.iFrameBlockTimer <= 0) { b.iFrameBlock = false; } }
    }
}
...
void UpdateProjectiles() {
    for (auto& p : projectiles) {
        ...
        if (p.x >= ballX - expandedSize && p.x <= ballX + expandedSize && p.y >= ballY - expandedSize && p.y <= ballY + expandedSize) { float hitOffset = (p.x - ballX) / expandedSize; ballVelX += hitOffset * -0.01f; ballVelY = 0.030f; p.active = false; }
...
void UpdateDash() {
    if (dashActive) {
        ...
        if (CircleRectCollision(ballX, ballY, ballSize, rx, ry, rw, rh)) {
            if (fabs(ballVelY * 1.2f) <= 0.03f) { ballVelY = 0.03f; }
            else { ballVelY = fabs(ballVelY * 1.2f); }
            float hitOffset = dashDir * -1.0f; ballVelX += hitOffset * -0.02f;
        }
        paddleHeight = paddleHeightDash; dashTimer -= 1; if (dashTimer <= 0) { dashActive = false; paddleHeight = paddleHeightNormal; } paddleX += (dashDir * dashSpeed);
    }
}

void UpdateGameplay() {
    if (stageTransitionTimer > 0) { stageTransitionTimer--; return; }
    if (life < 0) { selectedMenuIndex = 0; currentState = GameState::STATE_START_MENU; return; }
    ...
    if (blocksRemaining <= 0 && !modoEditor) {
        ...
        if (nextFile.is_open()) { nextFile.close(); stage++; InitStage(stage); }
        else { selectedMenuIndex = 0; currentState = GameState::STATE_START_MENU; return; }
    }

    static bool zWasPressed = false; static bool xWasPressed = false;
    if (GetAsyncKeyState('X') & 0x8000) {
        if (!xWasPressed) { if (!forceFieldActive) { if (GetAsyncKeyState(VK_LEFT) & 0x8000) { dashDir = -1.0f; ActivateDash(); } else if (GetAsyncKeyState(VK_RIGHT) & 0x8000) { dashDir = 1.0f; ActivateDash(); } else if (!dashActive) { ActivateforceField(); } } } xWasPressed = true;
    }
    else { xWasPressed = false; }

    if (GetAsyncKeyState('Z') & 0x8000) {
        if (!zWasPressed) { Projectile p; p.x = paddleX; p.y = paddleY + paddleHeight + 0.003f; p.active = true; projectiles.push_back(p); } zWasPressed = true;
    }
    else { zWasPressed = false; }

    ImGuiIO& io = ImGui::GetIO();
    if (!io.WantCaptureKeyboard) {
        if (!forceFieldActive && !dashActive) { if (GetAsyncKeyState(VK_LEFT) & 0x8000) paddleX -= 0.01f; if (GetAsyncKeyState(VK_RIGHT) & 0x8000) paddleX += 0.01f; }
    }
